...
**개인정보 경계.** 실시간 컨텍스트(`lib/liveContext`)는 현재 위치와 취침·기상 시각을 **그대로 넘기고**, 챗봇은 위치·수면 시각·집/학교 위치 질문에 답한다 — 2026-09-10에 막았다가 2026-09-16 본인 결정으로 다시 열었다. 생일은 여전히 나이 계산에만 쓰고 모델에는 연도만 준다(`lib/dateContext.ts`). “개인정보가 새는 버그”로 보고 다시 막지 말 것. 서버 캐시 4곳(live·blog·portfolio·about)은 `lib/cached.ts` 헬퍼를 쓴다.
