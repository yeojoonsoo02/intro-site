...
export default function SocialLinks({ colored = false, isDev = false }: { colored?: boolean; isDev?: boolean }) {
  ...
  const SNS = isDev
    ? [
      ...
      {
        href: 'https://www.instagram.com/chatgpt_krguide/',
        label: '개발자 인스타그램',
        color: '#e1306c',
        icon: (
      ...
      {
        href: 'https://pf.kakao.com/_dFqxdn',
        label: '개발자 카카오톡',
        color: '#fee500',
        icon: (
          <span className="relative flex items-center justify-center w-6 h-6">
      ...
      },
      {
        ...
        icon: (
          <span
            ...
            style={{ backgroundColor: githubColor + '22', borderRadius: '50%' }}
          >
            <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path
                fillRule="evenodd"
              ...
              C19.13 20.54 22 16.74 22 12.26 22 6.58 17.52 2 12 2z"
                className="fill-[#3f3f46] dark:fill-white"
              />
            </svg>
          </span>
        ),
